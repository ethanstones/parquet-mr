 ...
 *
 * This decoration is needed to add meta data to each column without changing existing class 'MessageType', which is used
 * extensively. Here is the example usage to add column metadata to schema with type of 'MessageType'. The metadata will
 * be useful for column access control.
 *
...
public class ExtType<T> extends Type {
  ...
  protected java.util.List<String[]> getPaths(int depth) {
    return this.type.getPaths(depth);
  }

  public <T> T convert(java.util.List<GroupType> path, TypeConverter<T> converter) {
